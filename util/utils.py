...
def read_data(data_dir: Union[str, Path] = "data",  
    ...
    # Read csv file
    csv_files = list(data_dir.glob("dfs*.csv"))
...
def plot(output, truth, step):
    plot_length = len(output)
    pyplot.plot(output,color="red")
    pyplot.plot(truth[:plot_length],color="blue")
    pyplot.plot(output-truth[:plot_length],color="green")
    pyplot.grid(True, which='both')
    pyplot.axhline(y=0, color='k')
    pyplot.savefig('output/transformer-step%d.png'%step)
    pyplot.close()
    return True
...
def predict_future(src, prediction):       
    ...
    obs_length = len(src)
    src = torch.cat((src, prediction)) # [m,m+1,..., m+n+output_window_size]

    output = src.cpu().view(-1)
    # I used this plot to visualize if the model pics up any long therm structure within the data.
    pyplot.plot(output,color="red")       
    pyplot.plot(output[:obs_length],color="blue")    
    pyplot.grid(True, which='both')
    pyplot.axhline(y=0, color='k')
    pyplot.savefig('output/transformer-future.png')
    # pyplot.show()
    pyplot.close()
    return True